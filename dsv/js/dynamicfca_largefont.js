function DynaSet()
{
    ...
    visParams = {
        ...
        //For linux ui image
        // intentLabelFontSize: 45,
        // layerLabelFontSize:40,

        //For tapered_edge image  
        intentLabelFontSize: 35,    
        layerLabelFontSize:25,

        intentLabelColorBoxSize: 20,
    ...
    function render(lattice, versions, visualization, isFirstRender, shouldAdjustFov)
    {
        ...
        // if (true)
        {
            ...
            if (visualization.type == "diff")
            {
                ...
                // .text(parseFloat(objectSizeScale[0]).toFixed(2) + " (in " + filenames[window.allVersions.indexOf(window.selectedVersions[0])].replace(".csv", "") +")");
                ...
                .attr("font-size", "10px")
                // .text(parseFloat(objectSizeScale[1]).toFixed(2) + " (in " + filenames[window.allVersions.indexOf(selectedVersions[1])].replace(".csv", "") +")" );
                .text(parseFloat(objectSizeScale[1]).toFixed(2));


                // ------------ size finishes, position begins
                // sizeLegendSvg.append("line")
                //         .attr("x1", 0)
                //         .attr("y1", 75)
                //         .attr("x2", bbox.width - 10)
                //         .attr("y2", 75)
                //         .attr("stroke", "grey")
                ...
                sizeLegendSvg.append('text')
                .attr("dx", function(d){return "20px"})
                .attr("dy", function(d){return "186px"})
                .attr("font-size", "10px")
                ...
                sizeLegendSvg.append('text')
                .attr("dx", function(d){return "20px"})
                .attr("dy", function(d){return "225px"})
                .attr("font-size", "10px")
                // .attr("font-weight", "bold")
                .text("Base set");
                ...
                sizeLegendSvg.append('text')
                .attr("dx", function(d){return "20px"})
                .attr("dy", function(d){return "251px"})
                .attr("font-size", "10px")
                // .attr("font-weight", "bold")
                .text('Set/Edge in '+"\"" + filenames[window.allVersions.indexOf(selectedVersions[0])].replace(".csv", "") +"\"");
                ...
                sizeLegendSvg.append("rect")
                    ...
                    .attr("stroke","black")
                    .attr("stroke-dasharray","5 2")
                    .attr("transform", "translate(-5,275)");

                    sizeLegendSvg.append('text')
                .attr("dx", function(d){return "20px"})
                .attr("dy", function(d){return "281px"})
                .attr("font-size", "10px")
                // .attr("font-weight", "bold")
                .text('Set/Edge in '+"\"" + filenames[window.allVersions.indexOf(selectedVersions[1])].replace(".csv", "") +"\"");
           ...
           // sizeLegendSvg.attr("transform","scale(2)");
           // sizeLegendSvg.attr("transform","translate(200,0)");
        ...
        }).attr(
        {
            ...
            return d.num ;
        }).attr("font-size", visParams.layerLabelFontSize)
        .attr("opacity", 1);
        ...
        if (visualization.hasLabels)
        {
           ...
           .attr("x", function(d)
            ...
            selections.conceptUpdate.select("g.intent").html(function(d)
            {
                ...
                {
                    var dy = index == 0 ? 0 : -visParams.intentLabelColorBoxSize;
                    pos = allConceptNames.indexOf(attr);
                    ...
                    {   var pad =2;
                        ...
                        var conceptColoredBoxPad =  (visParams.conceptWidth  - (allConceptNames.length * ((visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight))))/(allConceptNames.length+1) ;


                        var occupiedSpace = ((d.width / 2)-(visParams.conceptWidth/2) + allConceptNames.length * ((visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight)+conceptColoredBoxPad));
                        var freeSpace = visParams.conceptWidth - occupiedSpace;
                        var leftPaddingForCentering = freeSpace/2;
                        // result += "<rect class = \"textBackground\"  x=\"" + ((d.width / 2)-(bBox.width/2)) + "\" y=\"" + (- (visParams.intentLabelColorBoxSize+pad)*(index+1)) +"\" width=\""+ bBox.width+ "\" height=\"" + visParams.intentLabelColorBoxSize + "\" fill=\""+fillColor +"\">"  + "</rect>";
                        // result += "<rect class = \"textBackground\"  x=\"" + ( conceptColoredBoxPad + (d.width / 2)-(visParams.conceptWidth/2) + pos*((visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight)+conceptColoredBoxPad)) + "\" y=\"" + (- (visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight) - conceptTopPad - (pad)) +"\" width=\""+ (visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight) + "\" height=\"" + (visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight) + "\" fill=\""+fillColor +"\">"  + "</rect>";
                        result += "<rect class = \"textBackground\"  x=\"" + ( conceptColoredBoxPad + (d.width / 2)-(visParams.conceptWidth/2) + pos*((visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight)+conceptColoredBoxPad)) + "\" y=\"" + (- (visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight)-2 ) +"\" width=\""+ (visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight) + "\" height=\"" + (visParams.intentLabelColorBoxSize*visParams.intentNonLabelSmallRectHeight) + "\" rx=\"5\"" +  "\" ry=\"5\"" + "\" fill=\""+fillColor +"\">"  + "</rect>";
...

                        {
                            if(d.fullName.split(',').length ==1)    //Condition to check if it has only one name and no subset relationship
                            {
                                ...
                                result += "<text x=\"" + (d.width / 2) + "\" y=\"" + (- (visParams.intentLabelColorBoxSize)*(index+1) - conceptTopPad - (index*pad) - 6)   + "\" text-anchor= \"middle\" font-size=\"" +visParams.intentLabelFontSize  +"px \">" +attr;
                                result += "</text>";    
                            }
