function DynaSet()
{
    ...
    guiParams = {
        ...
        mainVisHeight: 50,
        // previewWidth: 80, // When editing, be sure to update CSS with half values t0o. #diffPreviewContainer {        padding-left: 40px;    }
        previewWidth: 130, // When editing, be sure to update CSS values t0o.
        previewHeight: 60,
        previewMargin: 10,
        previewBorder: 1,
    ...
    visParams = {
        ...
        // conceptWidth: 75,
        // conceptWidth: 150,
        // conceptWidth: 180, //teaser
        // conceptWidth: 200,
        conceptWidth: 360,
        minConceptHeight: 23,
        conceptMargin: 10, //default
        // conceptMargin: 100,
        conceptPadding: 4,
        dummyWidth: 5,
        dummyMargin: 3,
        ...
        objectMinSize: 5,
        // objectMinSize: 15,
        // objectMinSize: 10, //teaser
        // objectMinSize: 20, //diff_representation sec 5.4
        objectMaxSize: 20,
        // objectMaxSize: 30,
        objectMargin: 2,
        minLayerHeight: 50,
        ...
        // layerMargin: 100,
        // layerMargin: 120, //teaser
        layerMargin: 170,
        layerLabelsFontSize: 16,
        zoomStep: 1.05,
        minZoom: 0.2,
        maxZoom: 5,
        conceptCardinalityTextSize: 14,
        conceptCardinalityBoxSize: 30,
        statsMarginTop:18,
        statsBarHeight: 9,
        timeLineTopMargin: 8,
    ...
    {
        // $('#loader').modal('show');
        window.loadFunc = setTimeout(function(){d3.select("#loader2").style("display","block");}, 1);
        ...
        if(datasetIndex == 7)
            window.rowHeaders = window.rowHeaders_compResearch;
        else if(datasetIndex == 8)
            window.rowHeaders = window.rowHeaders_compResearchSmall;
        ...
        setTimeout(function(){d3.select("#loader2").style("display","none");}, 1);
    ...
    function constructGui(lattice, versions, versionLabels)
    {
        ...
        var mainVisDivHeight = d3.select("#mainVisualization").node().getBoundingClientRect().height - 50;
    ...
    function constructList(lattice, versions)
    {
        ...
        $(document).ready(function()
        {
            ...
            $('#listExample').DataTable(
            {
                ...
                // "scrollY": window.setViewHeight-100,
                "scrollY": "850px",
            ...
            d3.selectAll('#listExample tr').each(function(d,i)
            {
                if(i>0)
                {
                    d3.select(this).attr("objectName", dataForTable[i-1][0] );
                    d3.select(this).attr("sum", dataForTable[i-1][2] );
                }
            });
            ...
            d3.selectAll('#listExample tr').each(function(index)
            {
                d3.select(this).selectAll("td").each(function(d, i)
                {
                    ...
                    if(i==1)
                        d3.select(this).classed("sumColumn", true);
                    if(i==2)
                        d3.select(this).classed("timelineColumn", true);
            ...
            $('#listExample tr').addClass(function(index, currentClass)
            {
                ...
                var sumColumn = d3.select(this).attr("sum");
    ...
    function renderList(lattice, versions, type, objectIdArray)
    {
        ...
        var columnWidth = d3.select(".dataTableNameDiv").node().getBoundingClientRect().width;
        $('#listExample tbody tr').addClass(function(index, currentClass)
        {
            ...
            var objectName = d3.select(this).attr("objectName");
            d3.select(this.childNodes[0]).html("");
            // console.log(this.childNodes[0]);

            var div = d3.select(this.childNodes[0]).append("div").attr('style', 'margin-left:5px');
            div.text(objectName);

            var authorSvg = d3.select(this.childNodes[0]).insert("svg", "td");
            authorSvg.attr('style', 'background-color: white');

            // var columnWidth = div.node().getBoundingClientRect().width;
            // var columnWidth = 350;
            


            drawAuthorSetMembership(lattice, authorSvg, versions, objectName, maxContributionsInSelectedTimestamps, type, columnWidth);
        ...
        d3.selectAll("#listExample tr").on("mouseout", function(d, j)
        {
            ...
            var timelineBoxHeight = d3.select(".timelineColumn").node().getBoundingClientRect().height - 20;
            var leftPadding = 0;
            // var timelineBoxWidth = guiParams.authorTimelineWidth - leftPadding;
            var timelineBoxWidth = d3.select(".timelineColumn").node().getBoundingClientRect().width - 20;
            var heightScale = d3.scale.linear().domain([0, window.objectSizeScale[1]]).range([-4, (-timelineBoxHeight+2)]);
        ...
        $('#listExample tr').addClass(function(index, currentClass)
        {
            if (index > 0)
            {
                ...
                var data = d3.select(this).attr("objectName");
                var firstAppearanceOfObjectXPos = -1;
                ...
                if (objectEdge != undefined)
                {
                    ...
                    timelineGroup.append("polyline")
                        .attr(
                        {
                            "points": function(d)
                            {
                                ...
                                for (var i = 0; i < window.numberOftimesteps; i++)
                                {
                                    ...
                                    if((firstAppearanceOfObjectXPos == -1) && (objectData[versionNum] > 0) )
                                    {
                                         firstAppearanceOfObjectXPos = x;
                                        //  console.log(firstAppearanceOfObjectXPos);
                                    }
                                    // console.log(x,y, versionNum, objectData);
                                    // pointstring = pointstring.concat(x.toString().concat(",",y.toString()));
                                    pointstring += x + "," + y + " ";
                        ...
                        timelineGroup.append("line").attr({
                            "x1": firstAppearanceOfObjectXPos,
                            "y1":-4,
                            "x2": firstAppearanceOfObjectXPos,
                            "y2": timelineBoxHeight,
                            "stroke":"gray",
                            "stroke-width": "1px",
    ...
    function frequencyFromVersion(version)
        {
            ...
            for (var i = 0; i < window.allVersions.length; i++)
            {
                var possibleVersion = window.allVersions[i];
                if ((version & possibleVersion) > 0) count++;
            }
            return count / window.allVersions.length;
        }

    // Colored boxes beneath author names in element list
    function drawAuthorSetMembership(lattice, svg, versions, objectName, maxContributionsInSelectedTimestamps, type, columnWidth)
    {
        var objectEdge = lattice.objects[reverseObjectDict[objectName]];
        var roundCurve = 2;
        var xscale = d3.scale.ordinal()
                    .domain(allConceptNames)
                    .rangeRoundBands([ 0, columnWidth], 0.2);
        var setBoxHeight = 12;
        ...
        if (objectEdge != undefined)
        {
            ...
            svg.attr('width', columnWidth)
                .attr("height", setBoxHeight);
            ...
            if (versions.length == 1)
            {
                ...
                {
                    svg.append('rect')
                            .attr("class", "boundingSetBox")
                            .attr("x", xscale(allConceptNames[i]))
                            .attr("y", 1)
                            .attr("rx", roundCurve)
                            .attr("ry", roundCurve)
            ...
            else if (versions.length == 2 && type == "diff")
            {
                for (i = 0; i < 2; i++)
                {
                    ...
                    for(var k=0; k<allConceptNames.length; k++)
                    {
                        svg.append('rect')
                                .attr("class", "boundingSetBox")
                                .attr("x", xscale(allConceptNames[k]))
                                .attr("y", 1)
                                .attr("rx", roundCurve)
                                .attr("ry", roundCurve)
                    ...
                    conceptDictArray.forEach(function(attr, index)
                    {
                        pos = allConceptNames.indexOf(attr);
                        {
                            ...
                            svg.append('rect')
                                ...
                                .attr("x", function(){
                                    if(i==0)
                                        return (xscale(attr) + (i*xscale.rangeBand()/2));
                                    else
                                        return (xscale(attr) + (i*xscale.rangeBand()/2)) + 1;
                        ...
                        // svg.append("line")
                        // .attr("x1", xscale(attr)+ xscale.rangeBand() / 2)
                        // .attr("y1", 1)
                        // .attr("x2", xscale(attr)+ xscale.rangeBand() / 2)
                        // .attr("y2", setBoxHeight -2)
                        // .attr("style", "stroke:rgb(255,255,255);stroke-width:3");
                    });
            ...
            else
            {
                ...
                var objectId = -1;
                // for(var oid in lattice.objects)
                // {
                //     if(lattice.objects[oid].name == objectName )
                //     {
                //         objectId = oid;
                //         break;
                //     }
                // }
                objectId = parseInt(reverseObjectDict[objectName]);
                var conceptsObjectBelongsToArray = lattice.objects[objectId].concepts;
                var conceptNameToFrequencyDictionary = {};
                for(var k=0; k<conceptsObjectBelongsToArray.length; k++)
                {
                    var cid = conceptsObjectBelongsToArray[k].conceptId;
                    var cname = lattice.concepts[cid].fullName;
                    if(cname.indexOf(",") ==-1)
                    {
                        conceptNameToFrequencyDictionary[cname] = frequencyFromVersion(conceptsObjectBelongsToArray[k].version);
                    }
                }
                    ...
                    pos = allConceptNames.indexOf(attr);
                    {
                        ...
                        svg.append('rect')
                            ...
                            .attr("fill-opacity", function(){
                                return conceptNameToFrequencyDictionary[attr];
                            });
                    }
                });
                // svg.append("line")
        ...
        else if (type == "conceptSelection")
        {
            ...
            for(var k=0; k<window.selectedConceptIdForHighlight.length;k++)
            {
                ...
                tagSpan.append("i").attr("class", "icon-remove").attr("conceptIdTag",window.selectedConceptIdForHighlight[k]).on("click", function()
                {
                    ...
                    if(tid in window.objects["conceptSelection"])
    ...
    function drawTopTimeline(timelineDataDictAll, timelineDataDictSelection, isFirstRender)
    {
        ...
        if(isFirstRender)
        {
            ...
            var barsSelection = g.selectAll(".bar")
                    ...
                    {
                        return  "Total "+d + " elements in "+filenames[i];
                    });
        ...
        
        {
            ...
            .attr("title", function(d,i)
            {
                ...
                if(parseInt(d3.select("#selectedNumObjects").text()) == d)
                    ...
                    titleStr =  "Among "+d3.select("#selectedNumObjects").text()+" selected elements, "+d + " elements are present in "+filenames[i];
                return titleStr;
            });
            // .append("title").text(function(d,i)
            // {
            //     return filenames[i] +": "+ d + " elements";
            // });
    ...
    function calculateStatistics(objectIdArray)
    {
        ...
        // drawDegreeDistribution4(historgramDataArray, isDiff, window.largestDegree, historgramDataObjectsArray);
    }
    ...
    function drawDegreeDistribution(historgramDataArray, isDiff)
    {
        ...
        else
        {
            ...
            var bars = barsSelection.append("rect")
                ...
                {
                    if ((i % 2) == 0)
                        return "# elements in " + filenames[window.allVersions.indexOf(window.selectedVersions[0])].replace(".csv", "") + ": " + historgramDataArray[0][d];
                    else
                        return "# elements in " + filenames[window.allVersions.indexOf(window.selectedVersions[1])].replace(".csv", "") + ": " + historgramDataArray[1][d];
                });
            var radius = 5;
    ...
    function drawDegreeDistribution2(historgramDataArray, isDiff, largestDegree, historgramDataObjectsArray)
    {
        ...
        if (!isDiff)
        {
            ...
            var bars = barsSelection.append("rect")
                ...
                // .append("title").text(function(d)
                // {
                //     return "# elements of degree "+d+" : " + historgramDataArray[0][d];
                // });
                .attr("title", function(d)
                {
                    ...
                    // return "Among "+d3.select("#selectedNumObjects").text()+" selected elements,there are "+historgramDataArray[0][d]+" elements of degree "+d;
                    var titleStr = "";
                    if(parseInt(d3.select("#selectedNumObjects").text()) == historgramDataArray[0][d])
                        ...
                        titleStr =  "Among "+d3.select("#selectedNumObjects").text()+" selected elements, "+historgramDataArray[0][d] + " elements of degree "+d+" are present ";
        ...
        else
        {
            ...
            var bars = barsSelection.append("rect")
                ...
                // {
                //     if ((i % 2) == 0)
                //         return "# elements of degree "+d+" in " + filenames[window.allVersions.indexOf(window.selectedVersions[0])].replace(".csv", "") + ": " + historgramDataArray[0][d];
                //     else
                //         return "# elements of degree "+d+" in " + filenames[window.allVersions.indexOf(window.selectedVersions[1])].replace(".csv", "") + ": " + historgramDataArray[1][d];
                // });
                .attr("title", function(d, i)
                {
                    if ((i % 2) == 0)
                        return "Among "+d3.select("#selectedNumObjects").text()+" selected elements, there are "+ historgramDataArray[0][d]+" elements of degree "+d+" present in " + filenames[window.allVersions.indexOf(window.selectedVersions[0])].replace(".csv", "");
                    else
                        return "Among "+d3.select("#selectedNumObjects").text()+" selected elements, there are "+ historgramDataArray[1][d]+" elements of degree "+d+" present in " + filenames[window.allVersions.indexOf(window.selectedVersions[1])].replace(".csv", "");
    ...
    function drawDegreeDistribution4(historgramDataArray, isDiff, largestDegree, historgramDataObjectsArray)
    {
        var width = d3.select("#selectionStatisticsSVG").node().getBoundingClientRect().width ;
        var height = 40 + window.NumBaseSets*2*10 ;
        var margin = {
        ...
        // function bandNumber(n)
        // {
        //     return Math.floor(n/numberOfBands);
        // }
        ...
        var minY = 0, maxY = height - margin.top -margin.bottom;
        var numberOfBands = 3;
        var colorsOfBands = ["#7B7D7D", "#4D5656", "#000000"];
        var oneBand = window.maxObjectsInAnyDegree/numberOfBands;
        var bandYScale = d3.scale.linear().domain([0, oneBand]).range([0, height - margin.top -margin.bottom]);
        // y.domain([0, oneBand]);
        ...
        if (!isDiff)
        {
            ...
            var bars = barsSelection.append("rect")
                ...
                .attr("y", function(d)
                {
                    ...
                    // return height-margin.bottom - y(historgramDataArray[0][d]);
                    var tempVal = historgramDataArray[0][d];
                    console.log(height-margin.bottom - bandYScale(tempVal%numberOfBands));
                    return height-margin.bottom - bandYScale(tempVal%numberOfBands);
                ...
                .attr("fill", function(d){
                    var tempVal = historgramDataArray[0][d];
                    var bandNum = Math.floor(tempVal/numberOfBands);;
                    return colorsOfBands[bandNum -1];
                })
                .attr("height", function(d){ 
                    // return y(historgramDataArray[0][d]);
                    var tempVal = historgramDataArray[0][d];
                    // var bandNum = bandNumber(tempVal);
                    return bandYScale(tempVal%numberOfBands);
                })
                .attr("width", function(d)
                {
                    var tempVal = historgramDataArray[0][d];
                    var bandNum = Math.floor(tempVal/numberOfBands);;
                    return x.rangeBand() - bandNum*2;
                }).on("click", function(d,i){
                    if(window.selectedType!="diff" && window.selectedDegree == d)
                    {
    ...
    function constructNavigation(lattice, versions, versionLabels, navigateCallback)
    {
        ...
        d3.select("#individualVersionRow div").append("div").attr("id","navigationLegend").attr("style","float:left;width:45px;height:"+guiParams.previewHeight+"px;");
        
        

        // Create the selection bar.
        // var selectionBarContainer = navigationContainer.append("div").attr("id", "selectionBar");
        var selectionBarContainer = d3.select("#selectionBar").attr("style", "padding-left:60px;");
        ...
        for (var i = 0; i < versions.length; i++)
        {
            ...
            if (i < versions.length - 1)
            {
                ...
                //     "fill":"none"
                // }).append("title").text("# elements only in "+filenames[window.allVersions.indexOf(selectedVersions[0])].replace(".csv", ""));
                ...
                }).append("title").text("# elements only in " + filenames[window.allVersions.indexOf(versions[i])].replace(".csv", ""));
                ...
                }).append("title").text("# elements only in " + filenames[window.allVersions.indexOf(versions[i + 1])].replace(".csv", ""));
                ...
                }).append("title").text("# elements in " + filenames[window.allVersions.indexOf(versions[i])].replace(".csv", "") + " and " + filenames[window.allVersions.indexOf(versions[i + 1])].replace(".csv", ""));;
        ...
        d3.select("#navigationLegend").attr("style","float:left;width:45px;height:"+tempheight+"px;");

        var navigationLegendSVG = d3.select("#navigationLegend").append("svg").attr("width",guiParams.timelineScaleWidth).attr("height",tempheight);
        var top_bottommargin = (tempheight - guiParams.previewHeight)/2;
        navigationLegendSVG.append("line").attr({
            "x1":guiParams.timelineScaleWidth -1,
            "y1":top_bottommargin,
            "x2":guiParams.timelineScaleWidth-1,
        ...
        var tickG = navigationLegendSVG.append("g");
                ...
                {
                    tickG.append("text").text(tickValues[b]).attr({
                        "x":guiParams.timelineScaleWidth-8,
                    ...
                    ;
                    tickG.append("line").attr({
                        "x1":guiParams.timelineScaleWidth-6,
    ...
    function render(lattice, versions, visualization, isFirstRender, shouldAdjustFov)
    {
        ...
        // if (true)
        {
            ...
            if (visualization.type == "diff")
            {
                ...
                sizeLegendSvg.append('text')
                    ...
                    .attr("font-size", "10px")
                    .text('Highest intersection layer');
                sizeLegendSvg.append('text')
                    .attr("dx", function(d)
                    {
                        return "20px"
        ...
        selections.separatorUpdate.attr("text-anchor", "left")
            .attr(
            {
                ...
                x: (visualization.bbox.x2 + 20) //default
                // x: (visualization.bbox.x2 + 40)
                // x: (visualization.bbox.x2 - 30)  
            }).attr("y", function(d)
            {
        ...
        if (visualization.type == "diff")
        {
            ...
            selections.conceptCreate.append("rect").classed("concept", true).attr(
                ...
                .attr("width", function(d)
                {
                    return visParams.conceptCardinalityBoxSize
                    return d.width;
                }).attr("height", function(d)
                {
                    return visParams.conceptCardinalityTextSize + 2;
                    return d.height;
                }).attr(
                {
                    "fill": "white",
            ...
            selections.conceptCreate.append("text")
                .attr(
                {
                    ...
                    "text-anchor": "middle",
                    "fill": "black",
                    "font-size": visParams.conceptCardinalityTextSize
                })
                .text(function(d)
                {
                    var cardinality = 0;
                ...
                .attr(
                {
                    x: function(d)
                    {
                        return -2 + visParams.conceptCardinalityBoxSize/2;
                    },
                    y: function(d)
                    {
                        return d.height + 6 + (visParams.conceptCardinalityTextSize - 13)
                    }
                });
            ...
            selections.conceptCreate.append("rect").classed("concept", true).attr(
                {
                    x: function(d)
                    {
                        return d.width - visParams.conceptCardinalityBoxSize + 2
                    },
                    y: function(d)
                    {
                        return d.height - 2
            ...
            selections.conceptCreate.append("text")
                ...
                .attr(
                {
                    x: function(d)
                    {
                        return d.width + 2 - visParams.conceptCardinalityBoxSize/2
                    },
                    y: function(d)
                    {
                        return d.height + 6 + (visParams.conceptCardinalityTextSize - 13)
            ...
            {
                return "translate(" + (iconSpace / 2 - 1) + ", " + (d.height + visParams.statsMarginTop + visParams.statsBarHeight/2 +2) + ")"
            }).append("title").text("# elements only in " + filenames[window.allVersions.indexOf(versionA)].replace(".csv", ""));
            ...
            {
                return "translate(" + (iconSpace / 2 + 1) + ", " + (d.height + visParams.statsMarginTop + 3*visParams.statsBarHeight/2 + 4) + ")"
            }).append("title").text("# elements only in " + filenames[window.allVersions.indexOf(versionB)].replace(".csv", ""));
            ...
            {
                return "translate(" + (iconSpace / 2) + ", " + (d.height + visParams.statsMarginTop + 5*visParams.statsBarHeight/2 + 6) + ")"
            }).append("title").text("# elements in " + filenames[window.allVersions.indexOf(versionA)].replace(".csv", "") + " and " + filenames[window.allVersions.indexOf(versionB)].replace(".csv", ""));
            ...
            selections.conceptCreate.append("line")
                .attr(
                {
                    ...
                    y1: function(d)
                    {
                        return d.height + visParams.statsMarginTop + visParams.statsBarHeight/2
                    },
                    x2: iconSpace,
                    y2: function(d)
                    {
                        return d.height + visParams.statsMarginTop + 5*visParams.statsBarHeight/2 + 8
                    },
                    "stroke": "grey",
                    "stroke-width": 1
                })
            ...
            statsBarForPreviousTimestep.append("rect").classed("conceptStatsBar", true).attr(
                {
                    ...
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop + 2
                    },
                })
                .attr("width", function(d)
                {
                ...
                }).attr("height", visParams.statsBarHeight).append("title")
                .text(function(d)
                {
                    var cardinality = 0;
                    for (var i = 0; i < d.objectsFiltered.length; i++)
            ...
            statsBarForNextTimestep.append("rect").classed("conceptStatsBar", true).attr(
                {
                    ...
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop + visParams.statsBarHeight + 4
                    },
                })
                .attr("width", function(d)
                {
            ...
            statsBarForNextTimestep.append("text")
                ...
                .attr(
                {
                    ...
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop + 3*visParams.statsBarHeight/2 +2
                    }
                });
            ...
            statsBarForBothTimes.append("rect").classed("conceptStatsBar", true).attr(
                {
                    ...
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop + 2*visParams.statsBarHeight + 6
                    },
                })
                .attr("width", function(d)
                {
            ...
            statsBarForBothTimes.append("text")
                ...
                .attr(
                {
                    ...
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop + 5*visParams.statsBarHeight/2 + 4
                    }
                });
        ...
        else
        {
            ...
            {
                return "translate(" + (iconSpace / 2 - 1) + ", " + (d.height + visParams.statsMarginTop + visParams.statsBarHeight/2) + ")"
            });;
            ...
            {
                return "translate(" + (iconSpace / 2 - 1) + ", " + (d.height + visParams.statsMarginTop + 3*visParams.statsBarHeight/2 + 2) + ")"
            });
            ...
            conceptSelectionTemp.append("line")
                .attr(
                {
                    ...
                    y2: function(d)
                    {
                        return d.height + visParams.statsMarginTop + 2*visParams.statsBarHeight + 4
                    },
                    "stroke": "grey",
                    "stroke-width": 1
                })
            ...
            statsBarGroupWithHat.append("rect").classed("conceptStatsBar", true)
                .attr(
                {
                    ...
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop
                    },
                })
                .attr("width", function(d)
                {
                ...
                }).attr("height", visParams.statsBarHeight).append("title").text(function(d)
                {
            ...
            statsBarGroupWithHat.append("text")
                ...
                .attr(
                {
                    x: function(d)
                    {
                        return iconSpace + barScale(hatDataForVeryConcept[d.id].withHat) + 4
                    },
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop + visParams.statsBarHeight/2 - 2
                    }
                });
            ...
            statsBarGroupWithoutHat.append("text")
                ...
                .attr(
                {
                    x: function(d)
                    {
                        return iconSpace + barScale(hatDataForVeryConcept[d.id].withoutHat) + 4
                    },
                    y: function(d)
                    {
                        return d.height + visParams.statsMarginTop + 3*visParams.statsBarHeight/2
                    }
                });
            ...
            var timelineBoxHeight = 35;
            var leftPadding = 2*iconSpace;
            ...
            timelineGroup.append("line")
                .attr(
                {
                    ...
                    y1: function(d)
                    {
                        return d.height + 2*visParams.statsBarHeight + visParams.statsMarginTop + visParams.timeLineTopMargin
                    },
                    x2: leftPadding,
                    y2: function(d)
                    {
                        return d.height + 2*visParams.statsBarHeight + visParams.statsMarginTop+ visParams.timeLineTopMargin + timelineBoxHeight
                    },
                    "stroke": "#cccccc",
                    "stroke-width": 1
                })
                    ...
                    "points": function(d)
                    {
                        ...
                        for (var i = 0; i < window.numberOftimesteps; i++)
                        {
                            ...
                            if (versionNum in conceptData)
                                y = (d.height + 2*visParams.statsBarHeight + visParams.statsMarginTop + visParams.timeLineTopMargin + timelineBoxHeight) + heightScale(conceptData[versionNum]);
                            else
                                y = (d.height + 2*visParams.statsBarHeight + visParams.statsMarginTop + visParams.timeLineTopMargin + timelineBoxHeight);
                            pointstring += x + "," + y + " ";
            ...
            timelineGroup.style("visibility", function(d){
                if (versions.length != 1) return "visible";
                else return "hidden";
            });
        ...
        function appendObjectHalfcircles(objectCreateSelection, version)
        {
            ...
            {
                halfObject = objectCreateSelection.filter(function(d)
                {
                    return (d.version & version) > 0 ? this : null;
                });
                halfObject.append("path").attr("d", function(d)
                {
                    var path = "";
        ...
        if (visualization.type == "diff")
        {
            ...
            // console.log("#Objects climbed to higher layers: " + window.objectsClimbedUp + " # elements went to lower layers: "+window.objectsClimbedDown);
        ...
        for (var layer = minLayer; layer <= maxLayer; layer++)
        {
            ...
            if (layer == 1)
                ...
                separatorList.push(
                {
                    ...
                    num: "L"+layer+": "+layer + "-set intersections"
                });
        }
        return separatorList;
    }
