...
function renderNextFrame(stateArray, i, video)
{
    ...
    for(var j=0; j<stateArray[i]["agents"].length; j++)
    {
        
        var playerId = stateArray[i]["agents"][j]["agent_id"];
        var playerBoardValue = -1;
        switch(playerId)
        {
            case 0: playerBoardValue = 10; break;
            case 1: playerBoardValue = 11; break;
            case 2: playerBoardValue = 12; break;
            case 3: playerBoardValue = 13; break;
        }
        var player = d3.select("#Player"+playerBoardValue);
        
        var currentPos = stateArray[i]["agents"][j]["position"];
        var previousPos =  stateArray[i-1]["agents"][j]["position"];
        var newXPosBoard = currentPos[1]*window.svgWidthHeight/11;
        var newYPosBoard = currentPos[0]*window.svgWidthHeight/11;
        // var tempAgentStatusArray = [1,1,1,1];
        if(stateArray[i]["agents"][j]["is_alive"] == false) 
        {
            player.attr("visibility", "hidden");
        }
        else if(previousPos[0] == currentPos[0] && previousPos[1] == currentPos[1])
                continue;
        else if(previousPos[0] != currentPos[0])
            player.transition().duration(durationOfFrame).attr("y", newYPosBoard);
        else if (previousPos[1] != currentPos[1])
            player.transition().duration(durationOfFrame).attr("x", newXPosBoard);
    ...
    setTimeout(function(){
        for(var j=0; j<board.length; j++)
        {
            for(var k=0; k<board[j].length; k++)
            {
                ...
                {
                    var previousBoardValue = previousBoard[j][k];
                    var currentBoardValue = board[j][k];

                    var imagePath = returnImagePath(currentBoardValue);

                    if(currentBoardValue <=8)
                    {
                        d3.select("#Image"+j+"_"+k).attr("xlink:href",imagePath);
                    }
