function DynaSet()
{
    ...
    function separateExclusiveElements(inputContext)
    {
        ...
        for(var i=0; i< objectArray.length; i++)
        {
            ...
            {
                separatedContext[belongingAttributes[0]]["exclusive"].push({"objectid":objectArray[i] , "weight": inputContext.objects[objectArray[i]].weights[0]});
            }
            else if(belongingAttributes.length>1)
            {
                for(var l=0; l<belongingAttributes.length; l++)
                {
                    separatedContext[belongingAttributes[l]]["shared"].push({"objectid":objectArray[i]});
                }
                ...
                if(temphash in separatedContext)
                {
                    ...
                    separatedContext[temphash]["exclusive"].push({"objectid": objectArray[i], "weight":tempweight});
                }
                else
                {
                    var tempweight = calculateWeight( inputContext.objects[objectArray[i]].weights);
                    separatedContext[temphash]={"exclusive":[{"objectid": objectArray[i], "weight":tempweight}], "shared":[], "degree":tempsortedattrs.length };
    ...
    function computeGraphLayout ()
    {
        ...
        window.maxHeightOfNode = 10 + 50/numSets;
        window.widthOfNode = 50;
        var rightPadding = window.widthOfNode + 10;
        var paddingBetweenNodes = 5 + 30/numSets;;
       ...
       for(var i=0; i< degreeDictionary["1"].length; i++)
       {
        ...
        for(var i=0; i<inputRawData.length; i++)
        {
            ...
            for(var set in inputRawData[i])
            {
                if(!(set ==="added" || set === "removed"))
                {
                    ...
                    {
                        degreeDistributionPerTimeStep[i][tempDeg] = {"count": num, "objects": copy(inputRawData[i][set].exclusive)};
                    }
                }
                for(var j=1; j<=window.numSets; j++)
                {
                    if(!(j in degreeDistributionPerTimeStep[i]))
                        window.degreeDistributionPerTimeStep[i][j] = {"count":0, "objects":[]};
                }
            }
            ...
            {
                if(degreeDistributionPerTimeStep[i][deg].count > maxObjectsInAnyDegree)
                    maxObjectsInAnyDegree = degreeDistributionPerTimeStep[i][deg].count;
            }

        }
        var xscale = d3.scale.ordinal()
                        .domain(Object.keys(degreeDictionary))
        ...
        for(var i=0; i<inputRawData.length; i++)
        {
            ...
            for(var deg in degreeDistributionPerTimeStep[i])
            {
                degreeDistributionRow.append("rect").attr({
                    "x": function(){return xscale(deg)},
                    "y": (topPadding + heightOfDegreeDistributionBox) -  yscale(degreeDistributionPerTimeStep[i][deg].count),
                    "width": xscale.rangeBand(),
                    "height": function(){return yscale(degreeDistributionPerTimeStep[i][deg].count)},
                    "fill":"grey"
                });
                ...
                .append("title").text(function(){
                    ...
                    if(wt == 1) return wt+" element in exclusive "+deg+"-set intersections";
                    else return wt+" elements in exclusive "+deg+"-set intersections";
                });
            }
            degreeDistributionRow.append("rect").attr({
        ...
        degreeDistributionRow.attr("transform","translate("+(leftPadding+ksetGroupWidth-widthOfOneZone/4)+",0)");
        ...
        for(var i=0; i<inputRawData.length; i++)
        {
            ...
            var tlabel = timestepLabel.append("text").text(
                ...
                .attr({
                "x": labelXpos,
                "y": topPadding+heightOfDegreeDistributionBox+maxHeightOfNode,
                "text-anchor":"middle",
                "timestepIndex":i,
                "cursor":"pointer",
                "class":"timestepLabel",
            ...
            timestepLabel.append("rect").attr({
                ...
                "y": function(){
                        if(window.currentDatasetId != "image")
                            return topPadding+heightOfDegreeDistributionBox+maxHeightOfNode - bbox.height/2 - extra;
                        else
                            return topPadding+heightOfDegreeDistributionBox+maxHeightOfNode - bbox.height + extra/2 ;
                    },
            ...
            if(window.selectedTimestepForSorting == i && window.sortBy=="timestep")
                timestepLabel.append("svg:image")
                    ...
                    .attr('y', topPadding+heightOfDegreeDistributionBox+maxHeightOfNode - 20)
                    .attr('width', 20)
                    .attr('height', 30)
                    .attr('class', 'sortIcon')
                    .attr("xlink:href", "images/downArrow.png")
        ...
        timestepLabel.attr("transform","translate("+(leftPadding+ksetGroupWidth-widthOfOneZone/4)+",0)");
    ...
    function createElementList() {
        ...
        function sortObjects(objectArray, searchText)
        {
            ...
            for(var i=0; i<objectArray.length; i++)
            {
                if(searchText != '' && objectArray[i][4].toUpperCase().includes(searchText.toUpperCase()))
                {
            ...
            var rows = table.selectAll("tr.tablerow").data(objectArray).enter().append("tr")
                ...
                .on("click", function (dataitem) {
                    ...
                    if (!$(this).hasClass("highlighted")) 
                    {
                        ...
                        const elementName = allObjectsInfo[d].name;
                        if (datasetSelection.currentId === "image") {
                            ...
                            $("#highlightedElement").html(`<div>${elementName}</div>`);
                        }
                    } 
                    else 
                    {
